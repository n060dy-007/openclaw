    ...
    buildRun: () => {
      ...
      return {
        afterReply: async (_reply, context) => {
          ...
          await waitForStructuredReply("document", documentStartedAt, documentToken);
          ...
          await waitForStructuredReply("poll", pollStartedAt, pollToken);
          ...
          await waitForStructuredReply("location", locationStartedAt, locationToken);

          const contactStartedAt = new Date();
          const driverContactWaId = context.driverPhoneE164.replace(/\D/g, "");
          await context.driver.sendContact(context.target, {
            ...
            vcard: [
              ...
              `TEL;type=CELL;type=VOICE;waid=${driverContactWaId}:${context.driverPhoneE164}`,
              "END:VCARD",
            ].join("\n"),
          });
          await waitForStructuredReply("contact", contactStartedAt, contactToken);

          const stickerStartedAt = new Date();
          await context.driver.sendSticker(context.target, WHATSAPP_QA_ONE_PIXEL_WEBP, {
            mimetype: "image/webp",
          });
          await waitForStructuredReply("sticker", stickerStartedAt, stickerToken);
