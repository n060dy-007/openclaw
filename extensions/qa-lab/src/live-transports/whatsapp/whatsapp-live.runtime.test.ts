...
describe("WhatsApp QA live runtime", () => {
  ...
  it("sends a WhatsApp-routable contact card in the structured-message check", async () => {
    ...
    if (run.kind === "approval" || !run.afterReply) {
      throw new Error("structured WhatsApp scenario unexpectedly built a non-hook run");
    }
    const driver = {
    ...
    await run.afterReply({ messageId: "initial-1", text: "ok" }, {
      driver,
      driverPhoneE164: "+15550000001",
    ...
    expect(driver.sendContact).toHaveBeenCalledWith(
      "+15550000002",
  ...
  it("labels structured-message contact wait failures", async () => {
    ...
    const driver = {
      ...
      waitForMessage: vi
        .fn()
        .mockResolvedValueOnce(observedReply)
        .mockResolvedValueOnce(observedReply)
        .mockResolvedValueOnce(observedReply)
        .mockRejectedValueOnce(new Error("timed out waiting for WhatsApp QA driver message")),
    };

    await expect(
      run.afterReply({ messageId: "initial-1", text: "ok" }, {
        driver,
        driverPhoneE164: "+15550000001",
        recordObservedMessage: vi.fn(),
        sutPhoneE164: "+15550000002",
    ...
    expect(driver.sendSticker).not.toHaveBeenCalled();
  });
